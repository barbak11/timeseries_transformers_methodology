...
def setup_dirs():
    """Создать каталоги артефактов для специфики coldstart."""
    for d in (MODELS_DIR, PREDS_DIR, METRICS_DIR):
        d.mkdir(parents=True, exist_ok=True)


def task_artifact_paths(task_key: str):
    """Построить пути артефактов для подзадачи внутри coldstart.
...
def plot_short_metrics_dashboard(dfm, title="Специфика: короткие ряды", save_path=None, show=True):
    """Построить дашборд метрик для coldstart (RMSE/WAPE + времена).
...
def rest_open_days_on_first_test_day(df, test_start):
    """Восстановить `open_days` на первый день теста для каждого ряда.

    Args:
        df: Таблица с `rest_id`, `date_dt`, `open_days`.
        test_start: Дата начала теста.

    Returns:
        DataFrame `rest_id` → `open_days` на старте теста.
    """
...
def select_short_rest_ids(open_days_df):
    """Выбрать cold-start ряды по фильтру `open_days`.

    Args:
        open_days_df: Таблица с `rest_id` и `open_days`.
...
def load_classic_split(df, test_start):
    """Разделить данные на train/test по дате.

    Args:
        df: Таблица с `rest_id` и `date_dt`.
        test_start: Дата начала теста.
...
def xgboost_drop_columns(df):
    """Список колонок, которые нельзя использовать как признаки XGBoost.

    Args:
        df: DataFrame с колонками.

    Returns:
        Список колонок на исключение.
    """
    base = [
        TARGET_COL,
        "date_dt",
...
def run_xgboost_all_eval_short(
    ...
    *,
    ...
    """Обучить глобальный XGBoost без cold-start рядов и оценить на тесте.

    Args:
        short_rest_ids: Идентификаторы cold-start рядов.
        test_start: Дата начала теста.
        results: Список словарей метрик.
...
def _norm_static_value(val):
    """Нормализовать значение категориального признака для стабильного маппинга."""
    if pd.isna(val):
        return "__na__"
...
def _group_key_from_row(row, cols):
    """Собрать ключ группы по набору статических колонок."""
    return tuple(_norm_static_value(row.get(c, np.nan)) for c in cols)


def _build_group_scalers(df, test_start):
    """Построить статистики масштабирования таргета по группам.

    Args:
        df: Таблица ряда(ов) в long-формате.
        test_start: Дата начала теста.

    Returns:
        (scalers, global_stats) — словари со статистиками mean/std/count.
    """
...
def _resolve_group_scaler(row, scalers, global_stats):
    """Выбрать лучший доступный scaler для строки по иерархии уровней."""
    for cols in TFT_SCALE_GROUP_LEVELS:
...
def prepare_tft_series(df, test_start):
    """Подготовить данные для TFT: дневная сетка, признаки, масштабирование по группам.

    Args:
        df: Таблица с `rest_id`, `date_dt`, `TARGET_COL` и статическими/известными фичами.
        test_start: Дата начала теста.
...
class TFTWindowDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        Returns:
            (scat, snum, past_kf, fut_kf, target_z).
        """
...
def make_tft_windows(series_data, lookback, horizon, stride=1):
    """Сгенерировать список окон (sid, start) для обучения.

    Args:
        series_data: Данные по сериям.
        lookback: Длина истории.
        horizon: Горизонт.
...
class GRN(nn.Module):
    """Gated Residual Network (блок TFT)."""
...
class TFTModel(nn.Module):
    """Упрощённая реализация TFT для мультисериального прогнозирования."""
...
def train_tft(model, tr_loader, va_loader, epochs, lr, grad_clip, patience, device):
    """Обучить TFT с early stopping по val loss.

    Args:
        model: Модель TFT.
        tr_loader: DataLoader train.
        va_loader: DataLoader val.
        epochs: Число эпох.
...
def infer_tft(model, series_data, short_rest_ids, lookback, horizon, device):
    """Сделать прогноз TFT для cold-start рядов на тестовом хвосте.

    Args:
        model: Обученная TFT модель.
        series_data: Подготовленные данные серий.
        short_rest_ids: Идентификаторы cold-start рядов.
        lookback: Длина истории.
        horizon: Горизонт.
        device: Устройство.
    ...
    def _take_past_with_left_pad(arr2d, end_pos, win_len):
        """Взять окно прошлого, дополняя нулями слева при нехватке истории."""
        st = end_pos - win_len
        if st >= 0:
...
def run_tft_short(
    ...
    *,
    ...
    """Запустить TFT для coldstart: подготовка, обучение и инференс.
...
def run_xgboost_disease_short_snapshot(
    ...
    *,
    ...
    """Обучить XGBoost для disease (snapshot) без cold-start рядов и оценить на cold-start.

    Args:
        short_ids: Идентификаторы cold-start рядов.
        test_start_snap: Граница по `snapshot_dt`.
        results: Список словарей метрик.
        all_predictions: Список DataFrame предсказаний.
...
def run_tft_disease_short(
    ...
    *,
    ...
    """Запустить TFT для disease в режиме cold-start (weekly).
...
def drop_weather_from_classic(df):
    """Удалить погодные признаки из classic-таблицы.
...
def truncate_disease_train_random_weeks(train_df, short_ids, min_weeks=50, max_weeks=150, seed=RANDOM_SEED):
    """Прокси к общей функции усечения disease-истории для coldstart.
...
def select_short_rest_ids(open_days_df):
    """Прокси выбора cold-start рядов по `open_days`.
...
def run_short_experiment(
    ...
    """Запустить coldstart-эксперимент для traffic и (опционально) disease.

    Keyword Args:
        do_xgb: Запуск XGBoost.
        do_tft: Запуск TFT.
        do_disease: Запуск disease-пайплайна.
        run_traffic_task: Запуск подзадачи traffic.
        run_disease_task: Запуск подзадачи disease.
