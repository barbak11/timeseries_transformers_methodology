...
def setup_dirs():
    """Создать каталоги артефактов для специфики short."""
    for d in (MODELS_DIR, PREDS_DIR, METRICS_DIR):
        d.mkdir(parents=True, exist_ok=True)


def task_artifact_paths(task_key: str):
    """Построить пути артефактов для подзадачи внутри short.

    Args:
        task_key: Ключ подзадачи (`traffic` или `disease`).
...
def log_short_result(results, all_predictions, model_name, pred_df, train_sec, infer_sec):
    """Добавить результат модели (RMSE/WAPE + времена) и сохранить предсказания.
...
def plot_short_metrics_dashboard(dfm, title="Специфика: короткие ряды", save_path=None, show=True):
    """Построить дашборд метрик для short (RMSE/WAPE + времена).
...
def save_metrics_files(results, spec_key=SPEC_KEY, metrics_dir=METRICS_DIR):
    ...
    Args:
        results: Список словарей метрик.
        spec_key: Префикс имени файла.
        metrics_dir: Каталог сохранения.
...
def to_daily_series(part, value_col=TARGET_COL):
    ...
    Args:
        part: DataFrame с колонкой `date_dt` и целевым столбцом.
...
def compute_test_start_by_ratio(df, date_col="date_dt", train_ratio=0.70, val_ratio=0.15):
    """Оценить границу теста как квантиль по уникальным датам.
...
def select_shortest_share_ids(train_df, share=0.10, id_col="rest_id"):
    """Выбрать долю рядов с самой короткой историей.

    Args:
        train_df: Train-таблица.
        share: Доля рядов.
        id_col: Колонка идентификатора.
...
def truncate_disease_train_random_weeks(train_df, short_ids, min_weeks=50, max_weeks=150, seed=RANDOM_SEED):
    ...
    Args:
        train_df: Train-таблица.
        short_ids: Идентификаторы, для которых укорачиваем историю.
...
def coerce_year_week_to_datetime(s: pd.Series) -> pd.Series:
    """Привести year-week строки к datetime.
...
def load_disease_classic_long():
    """Загрузить disease sequential dataset в недельном long-формате.

    Returns:
        DataFrame с колонками `rest_id`, `series_id`, `date_dt`, `target`.
    """
...
def rest_open_days_on_first_test_day(df, test_start):
    """Получить open_days на первый календарный день теста.

    Если строки на точную дату нет, берётся ближайшая следующая дата в данных.
...
def select_short_rest_ids(open_days_df):
    ...
    Returns:
        Отсортированный список `rest_id`.
    """
...
def load_classic_split(df, test_start):
    """Разделить classic-таблицу на train/test по дате.
...
def run_ets_short(train_df, test_df, short_rest_ids, results, all_predictions, *, preds_dir=PREDS_DIR):
    """Запустить ETS только по коротким рядам.

    Args:
        train_df: Train-таблица.
        test_df: Test-таблица.
        short_rest_ids: Список коротких `rest_id`.
...
def run_sarimax_short(train_df, test_df, short_rest_ids, results, all_predictions, *, preds_dir=PREDS_DIR):
    """Запустить SARIMAX только по коротким рядам.
...
def xgboost_drop_columns(df):
    """Сформировать список колонок, которые нужно исключить из признаков XGBoost.

    Args:
        df: Таблица boosting.

    Returns:
        Список колонок для drop.
    """
    base = [
        TARGET_COL,
        "date_dt",
...
def run_xgboost_all_eval_short(
    ...
    *,
    ...
    """Обучить XGBoost (общая модель) и оценить на коротких рядах теста.
...
class LSTMReg(nn.Module):
    """LSTM-регрессия для одномерного ряда."""
...
def run_lstm_global_train_short_infer(
    ...
    *,
    ...
    """Обучить LSTM глобально и оценить на коротких рядах.

    Обучение идёт на всех рядах с достаточной историей, а инференс и метрики считаются
    только для `short_rest_ids`.

    Args:
        train_df: Train-таблица.
        test_df: Test-таблица.
        short_rest_ids: Идентификаторы коротких рядов.
        results: Список метрик.
        all_predictions: Список предсказаний.
        retrain: Переобучать модель.
        models_dir: Каталог моделей.
    ...
    def _train_with_es(model, loader, X_val_np, y_val_np):
        ...
        Args:
            ...
            X_val_np: Валидационные окна.
            y_val_np: Валидационные цели.

        Returns:
            Лучший val RMSE.
        """
        def _val_rmse_in_batches():
            """Посчитать RMSE по val батчами (экономит память)."""
            preds = []
            model.eval()
            with torch.no_grad():
...
def prepare_tft_series(df, test_start):
    """Подготовить данные для TFT: нормализация и представление по рядам.
...
class TFTWindowDataset(torch.utils.data.Dataset):
    """Dataset окон для обучения/валидации TFT."""

    def __init__(self, series_data, windows, lookback, horizon):
        """Инициализировать dataset окон.

        Args:
            series_data: Данные серий.
            windows: Список окон (sid, start).
    ...
    def __getitem__(self, i):
        """Вернуть одно окно для обучения.

        Args:
            i: Индекс окна.

        Returns:
            Кортеж тензоров для TFT.
        """
...
def make_tft_windows(series_data, lookback, horizon, stride=1):
    """Сформировать список стартовых позиций окон для TFT.

    Args:
        series_data: Данные по рядам.
        lookback: Длина lookback.
        horizon: Горизонт.
        stride: Шаг генерации.

    Returns:
        Список окон в виде (series_id, start_idx).
    """
    wins = []
...
class GRN(nn.Module):
    """Gated Residual Network блок для TFT."""

    def __init__(self, d_in, d_h, d_out=None, dropout=0.1):
        """Инициализировать GRN.

        Args:
            d_in: Входная размерность.
            d_h: Размерность скрытого слоя.
            d_out: Выходная размерность.
            dropout: Dropout.
        """
        super().__init__()
        d_out = d_out or d_in
...
class TFTModel(nn.Module):
    """Модель TFT для краткосрочного прогноза по рядам."""
    ...
    def __init__(
        ...
        """Инициализировать TFT модель.

        Args:
            n_series: Число рядов (для эмбеддинга).
            cat_dims: Размерности категориальных статических признаков.
            n_snum: Число числовых статических признаков.
            n_past_num: Число наблюдаемых признаков прошлого.
            n_kf: Число известных признаков (past/future).
            lookback: Длина истории.
            horizon: Горизонт.
    ...
    def forward(self, sidx, scat, snum, past_num, past_kf, fut_kf):
        ...
        Args:
            sidx: Индексы рядов shape (B,).
            scat: Статические категориальные признаки.
            snum: Статические числовые признаки.
            past_num: Наблюдаемые признаки прошлого.
            past_kf: Известные признаки прошлого.
            fut_kf: Известные признаки будущего.

        Returns:
            Прогноз в z-пространстве shape (B, horizon).
        """
        B = sidx.size(0)
...
def train_tft(model, tr_loader, va_loader, epochs, lr, grad_clip, patience, device):
    """Обучить TFT с early stopping по RMSE на валидации.

    Args:
        model: Модель TFT.
        tr_loader: DataLoader для train.
        va_loader: DataLoader для val.
        epochs: Максимум эпох.
        lr: Learning rate.
        grad_clip: Клиппинг градиента.
        patience: Параметр early stopping.
...
def infer_tft(model, series_data, short_rest_ids, lookback, horizon, device):
    """Сделать прогноз TFT по тесту для коротких рядов.

    Args:
        model: Обученная модель.
        series_data: Данные по рядам из `prepare_tft_series`.
        short_rest_ids: Идентификаторы коротких рядов.
        lookback: Длина lookback.
...
def run_tft_short(
    ...
    *,
    ...
    """Запустить TFT для short-постановки и сохранить предсказания/метрики.
...
def run_xgboost_disease_short_snapshot(
    ...
    *,
    ...
    """Запустить XGBoost для disease snapshot задачи в short-постановке.

    Args:
        short_ids: Идентификаторы коротких рядов.
        test_start_snap: Дата разделения по snapshot_dt.
        results: Список метрик.
        all_predictions: Список предсказаний.
        retrain: Переобучать модель.
        models_dir: Каталог моделей.
...
def run_lstm_disease_short(
    ...
    *,
    ...
    """Запустить LSTM для disease sequential задачи в short-постановке.

    Args:
        train_df: Train-таблица disease.
        test_df: Test-таблица disease.
        short_ids: Идентификаторы коротких рядов.
        results: Список метрик.
        all_predictions: Список предсказаний.
        retrain: Переобучать модель.
    ...
    class _L(nn.Module):
        """Локальная LSTM модель для disease-окна (внутренняя)."""
        def __init__(self, hidden, layers, dropout):
            """Инициализировать внутреннюю LSTM."""
            super().__init__()
    ...
    def _fit_es(model, loader, Xv, yv):
        """Обучить внутреннюю LSTM с early stopping по RMSE на val."""
        opt = torch.optim.Adam(model.parameters(), lr=LSTM_LR)
        loss_fn = nn.L1Loss()
...
def run_tft_disease_short(
    ...
    *,
    ...
    """Запустить TFT для disease задачи в short-постановке.
...
def truncate_disease_train_random_weeks(train_df, short_ids, min_weeks=50, max_weeks=150, seed=RANDOM_SEED):
    """Прокси к общей функции усечения disease-рядов в `common.short_shared`.

    Args:
        train_df: Train-таблица.
        short_ids: Идентификаторы рядов для усечения.
        min_weeks: Минимальная длина.
        max_weeks: Максимальная длина.
        seed: Seed.
...
def select_short_rest_ids(open_days_df):
    """Прокси к общей функции отбора коротких рядов в `common.short_shared`.

    Args:
        open_days_df: Таблица `rest_id` × `open_days`.
...
def run_short_experiment(
    ...
    """Запустить полный пайплайн для short-постановки (traffic + disease).
    ...
    Args:
        ...
        do_lstm: Запуск LSTM.
        do_tft: Запуск TFT.
        do_disease: Выполнять disease-задачу.
        run_traffic_task: Выполнять traffic-задачу.
        run_disease_task: Выполнять disease-задачу.
        show_plot: Показать графики.
        retrain: Переобучать модели (если None — берётся из конфига).
    """
