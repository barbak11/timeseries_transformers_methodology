...
Флаг RETRAIN (ниже): при True — обучить LSTM и 3 конфигурации MV-Transformer и сохранить веса в
...
MVT_CONFIGS: tuple[dict[str, str | int | float], ...] = SC["MVT_CONFIGS"]
...
def paths_for_task(task: TaskSpec):
    """Пути артефактов (models/predictions/metrics) для задачи.

    Args:
        task: Спецификация задачи.

    Returns:
        (root, models_dir, preds_dir, metrics_dir).
    """
...
def setup_dirs(task: TaskSpec):
    """Создать каталоги артефактов для задачи.
...
def series_file_slug(series_id) -> str:
    """Стабильный slug для имени файла по `series_id`.

    Args:
        series_id: Идентификатор ряда.

    Returns:
        Строка для безопасного имени файла.
    """
    s = str(series_id)
...
def log_result(
    ...
    *,
    ...
    """Добавить метрики модели и сохранить предсказания в общий список.

    Args:
        results: Список словарей метрик.
        all_predictions: Список DataFrame предсказаний.
        model_name: Имя модели.
        pred_df: Таблица предсказаний с `y_true`/`y_pred`.
        train_sec: Время обучения (сек).
        infer_sec: Время инференса (сек).

    Keyword Args:
        tuning_time_sec: Время тюнинга/поиска гиперпараметров (сек).
    """
...
def save_metrics_files(task: TaskSpec, results):
    """Сохранить метрики задачи в CSV и JSON.
...
def _safe_qcut_3(values: pd.Series, labels: list[str]) -> pd.Series:
    """Тертили с fallback, если qcut неприменим.

    Args:
        values: Значения для бининга.
        labels: Метки 3 бинов.

    Returns:
        Серия строк с метками бинов.
    """
...
def _run_key_model_slice_analysis(task: TaskSpec, train_df: pd.DataFrame, results, all_predictions):
    ...
    def _agg_slice(df: pd.DataFrame, grp_col: str) -> pd.DataFrame:
        """Агрегировать метрики по группам для slice-анализа.

        Args:
            df: Таблица предсказаний.
            grp_col: Колонка группы (level/vol/horizon).

        Returns:
            Таблица метрик по группам.
        """
        rows = []
...
def to_regular_series(part: pd.DataFrame, task: TaskSpec, value_col: str | None = None):
    """Привести ряд к регулярной частоте (D/W) и заполнить пропуски нулями.

    Args:
        part: Срез данных одной серии.
        task: Спецификация задачи.
        value_col: Колонка значений (по умолчанию `task.value_col`).

    Returns:
        Pandas Series на регулярной сетке.
    """
...
def test_start_from_df(df: pd.DataFrame, task: TaskSpec):
    """Вычислить дату начала теста как последний год данных.

    Args:
        df: Таблица с колонкой `date_dt`.
        task: Спецификация задачи.
...
def compute_split_dates_by_ratio(
    ...
    """Рассчитать (val_start, test_start) по уникальным датам без утечки.
...
def sarima_grid_for_task(task: TaskSpec, seasonal_period: int) -> list[tuple[tuple[int, int, int], tuple[int, int, int, int]]]:
    """Выбрать сетку SARIMA в зависимости от частоты задачи.
    ...
    Returns:
        Список пар (order, seasonal_order).
    """
...
def _sarima_mle_converged(res) -> bool:
    """Проверить флаг сходимости MLE у SARIMAX результата."""
...
def _fit_sarima(
    ...
    *,
    ...
    """Обучить SARIMAX без exog устойчивым образом.
    ...
    Keyword Args:
        maxiter: Максимум итераций оптимизатора.

    Returns:
        Объект результата SARIMAX.
    """
    maxiter = int(maxiter or SARIMA_MAXITER)

    def _spec():
        """Сконструировать SARIMAX спецификацию для текущих порядков."""
        return SARIMAX(
            endog,
            order=order,
...
def load_correlated_rest_ids(path=CLUSTER_CSV):
    """Загрузить список `rest_id` (или `series_id`) кластера.

    Args:
        path: CSV с колонкой `rest_id` или `series_id`.

    Returns:
        Отсортированный список id.
    """
    df = pd.read_csv(path)
...
def drop_weather(df):
    """Удалить погодные признаки из таблицы.

    Args:
        df: Таблица с возможными погодными колонками.
...
def load_classic_cluster_split(cluster_ids):
    """Загрузить classic-данные и отфильтровать только ряды текущего кластера.

    Args:
        cluster_ids: Идентификаторы рядов кластера.

    Returns:
        DataFrame classic по выбранным `rest_id`.
    """
...
def load_disease_classic_long():
    """Загрузить disease в long-формате и собрать `rest_id` для рядов.

    Returns:
        DataFrame со столбцами `rest_id`, `date_dt`, `target` и признаками.
    """
...
def _coerce_year_week_to_datetime(s: pd.Series) -> pd.Series:
    """Привести `YYYY-W` (год-неделя) к датам (понедельник недели).

    Args:
        s: Серия со строками/датами.
...
def load_task_data(task: TaskSpec, cluster_ids: list | None = None):
    """Загрузить данные задачи и сделать временной split train/test.

    Args:
        task: Спецификация задачи.
        cluster_ids: Идентификаторы кластера (обязательны для traffic).

    Returns:
        (df_full, train_df, test_df, test_start, cluster_ids).
    """
...
def run_ets(task: TaskSpec, train_df, test_df, results, all_predictions):
    """Запустить ETS по каждой серии и собрать предсказания на тесте.
...
def run_sarima(
    ...
    def _fit_predict_one(series_id):
        """Подобрать (order, seasonal_order) по val и вернуть прогноз на тесте."""
...
def run_xgboost(
    ...
    *,
    ...
    """Обучить XGBoost (с Optuna) и посчитать прогноз на тесте.

    Args:
        task: Спецификация задачи.
        cluster_ids: Идентификаторы кластера (для traffic).
        val_start: Граница train_fit/val по `split_col`.
        test_start: Граница val/test по `split_col`.
        results: Список словарей метрик.
        all_predictions: Список DataFrame предсказаний.

    Keyword Args:
        xgb_data_parquet: Источник данных parquet.
        xgb_df: Данные напрямую DataFrame (если уже загружены).
        model_label: Имя модели в логах/метриках.
        model_file_suffix: Суффикс файла модели.
        predictions_filename: Имя файла предсказаний.
    """
    _, models_dir, preds_dir, _ = paths_for_task(task)
    pred_out = (
    ...
    def _fill_xy(tb):
        """Подготовить X/y для XGBoost с медианным заполнением NaN."""
    ...
    def objective(trial: optuna.Trial) -> float:
        """Optuna objective: минимизация RMSE на val."""
        params = {
    ...
    def _optuna_callback(study: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        """Печатать прогресс Optuna по мере завершения trials."""
        v = trial.value
...
class LSTMReg(nn.Module):
    """Простой LSTM-регрессор по одному каналу (target)."""
...
def _build_windows(series_norm, seq_len: int):
    """Собрать окна (X, y) длиной `seq_len` из нормализованного ряда.

    Args:
        series_norm: Нормализованный ряд.
        seq_len: Длина окна истории.

    Returns:
        (x_list, y_list) — списки окон и целевых значений.
    """
    x_list, y_list = [], []
...
def _lstm_train_with_es(
    ...
    """Обучить LSTM с early stopping по RMSE на val.

    Args:
        model: LSTM модель.
        loader: DataLoader train окон.
        X_va: Валидационные окна.
        y_va: Валидационные цели.
        device: Устройство.
...
def run_lstm(
    ...
    *,
    ...
    """Запустить LSTM: подбор конфигурации по val и прогноз на тесте.

    Args:
        task: Спецификация задачи.
        train_df: Train-таблица.
        val_start: Граница train_fit/val.
        test_df: Test-таблица.
        results: Список словарей метрик.
        all_predictions: Список DataFrame предсказаний.

    Keyword Args:
        retrain: Переобучать модель.
    """
...
def _tft_known_from_dates(dti: pd.DatetimeIndex) -> np.ndarray:
    """Построить базовые календарные признаки из дат.

    Args:
        dti: Индекс дат.

    Returns:
        Массив признаков shape (T, 4).
    """
...
def _tft_static_cols(task: TaskSpec, df: pd.DataFrame) -> list[str]:
    """Выбрать набор статических признаков для TFT по задаче.

    Args:
        task: Спецификация задачи.
        df: Таблица с признаками.
...
def _tft_series_static_map(df: pd.DataFrame, static_cols: list[str]) -> dict[str, np.ndarray]:
    """Построить карту `series_id` → вектор статических признаков.

    Args:
        df: Таблица train.
        static_cols: Колонки статических признаков.

    Returns:
        Словарь `series_id` → np.ndarray.
    """
...
class TFTReg(nn.Module):
    """Компактная TFT-подобная модель (LSTM + attention) для одного шага."""

    def __init__(self, obs_dim: int, known_dim: int, static_dim: int, hidden: int, layers: int, dropout: float):
        """Инициализировать TFTReg.

        Args:
            obs_dim: Размерность наблюдаемых признаков прошлого.
            known_dim: Размерность известных будущих признаков.
            static_dim: Размерность статических признаков.
            hidden: Размер скрытого состояния.
            layers: Число LSTM-слоёв.
            dropout: Dropout.
        """
    ...
    def forward(self, past_obs: torch.Tensor, known_fut: torch.Tensor, static_x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            past_obs: Прошлые наблюдения shape (B, L, obs_dim).
            known_fut: Известные признаки будущего shape (B, known_dim).
            static_x: Статические признаки shape (B, static_dim).

        Returns:
            Прогноз на следующий шаг shape (B, 1).
        """
...
def _tft_make_windows(
    ...
    """Собрать окна TFT (past_obs + known + static) с pooled-обучением.

    Args:
        train_df: Train-таблица.
        val_start: Граница train_fit/val.
        task: Спецификация задачи.
        lookback: Длина истории.

    Returns:
        Словарь массивов окон и статистик нормализации или None.
    """
...
def _tft_train_with_es(model: TFTReg, loader, Xv_p, Xv_k, Xv_s, yv, device: str) -> float:
    """Обучить TFTReg с early stopping по RMSE на val.

    Args:
        model: TFTReg модель.
        loader: DataLoader train окон.
        Xv_p: Валидационные past окна.
        Xv_k: Валидационные known признаки.
        Xv_s: Валидационные static признаки.
        yv: Валидационные цели.
        device: Устройство.
...
def run_tft(
    ...
    *,
    ...
    """Запустить TFT: обучение по пулу окон и прогноз на тесте.
...
def _default_cal_from_index(all_dates: pd.DatetimeIndex) -> np.ndarray:
    """Построить календарные признаки по индексу дат.

    Args:
        all_dates: Индекс дат.

    Returns:
        Массив календарных признаков shape (T, n_cal).
    """
    dd = all_dates
    cal_df = pd.DataFrame(
        {
...
class MVGroupDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, panel, windows, lookback, horizon):
        """Инициализировать dataset окон для MV-Transformer.

        Args:
            panel: Панель данных.
            windows: Список стартовых индексов.
            lookback: Длина истории.
            horizon: Горизонт.
        """
        self.p = panel
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            (past_z, past_cal, future_cal, future_y).
        """
        t = self.wins[idx]
        pe = t + self.L
        fe = pe + self.H
...
def _make_mv_windows(panel, lookback: int, horizon: int):
    """Сгенерировать индексы окон для обучения MV-Transformer.

    Args:
        panel: Панель, возвращаемая `_build_mv_panel`.
        lookback: Длина истории.
        horizon: Горизонт.

    Returns:
        Список стартовых индексов окон.
    """
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d, max_len=2048):
        """Инициализировать позиционное кодирование.
    ...
    def forward(self, x):
        """Добавить позиционное кодирование к входу.

        Args:
            x: Тензор shape (B, T, d).
...
def _mvt_encoder_layer(d: int, heads: int, ff: int, dropout: float) -> nn.TransformerEncoderLayer:
    """Сконструировать слой encoder для MVT."""
...
class TemporalConvStem(nn.Module):
    ...
    def __init__(self, d: int, kernel: int = 5, drop: float = 0.05):
        """Инициализировать темпоральный conv-stem.

        Args:
            d: Размерность эмбеддинга.
            kernel: Размер ядра свёртки.
            drop: Dropout.
        """
        super().__init__()
        pad = kernel // 2
...
class TemporalBlock(nn.Module):
    """Темпоральный encoder-блок (self-attention по времени)."""

    def __init__(self, d, heads, ff, dropout, n_layers):
        """Инициализировать темпоральный блок."""
        super().__init__()
...
class CrossSeriesBlock(nn.Module):
    """Cross-series encoder-блок (self-attention по сериям)."""

    def __init__(self, d, heads, ff, dropout, n_layers):
        """Инициализировать cross-series блок."""
        super().__init__()
        layer = _mvt_encoder_layer(d, heads, ff, dropout)
        self.enc = nn.TransformerEncoder(layer, num_layers=n_layers)
...
class MVCrossSeriesTransformer(nn.Module):
    """MV-Transformer: attention по времени и по сериям для панели [T,S]."""
    ...
    def __init__(
        ...
        """Инициализировать MV-Transformer.

        Args:
            n_series: Число рядов в группе.
            n_cal: Число календарных признаков.

        Keyword Args:
            lookback: Длина истории.
            horizon: Горизонт.
            d: Размерность скрытого представления.
            ff: Размер FFN.
            heads_t: Число голов по времени.
            heads_s: Число голов по сериям.
            n_temp: Число темпоральных слоёв.
            n_cross: Число cross-series слоёв.
            dropout: Dropout.
        """
        super().__init__()
    ...
    def forward(self, past_z, past_cal, future_cal):
        ...
        Args:
            past_z: Нормализованный таргет прошлого shape (B, L, S).
            past_cal: Календарь прошлого shape (B, L, n_cal).
            future_cal: Календарь будущего shape (B, H, n_cal).
...
def _train_mv_transformer(
    ...
    *,
    ...
    """Обучить MV-Transformer по train-окнам (OneCycleLR + AdamW).

    Args:
        model: Модель.
        tr_loader: DataLoader train окон.
        device: Устройство.

    Keyword Args:
        run_label: Префикс в логах.
...
def _infer_mv_transformer(model, panel, device, lookback: int, horizon: int):
    """Сделать автогресcивный прогноз MV-Transformer на тестовой части панели.
...
def run_mv_transformer(
    ...
    *,
    ...
    """Запустить MV Cross-Series Transformer для задачи (3 конфигурации из `config.py`).

    Args:
        task: Спецификация задачи.
        df_full: Полные данные задачи.
        cluster_ids: Идентификаторы кластера/группы рядов.
...
def run_correlated_experiment(**kwargs):
    """Обратная совместимость: запуск только traffic."""
...
def run_all_tasks(
    ...
    **kwargs,
    ...
    """Запустить correlated для traffic и disease последовательно.

    Keyword Args:
        do_ets: Запуск ETS.
        do_sarima: Запуск SARIMA.
        do_xgb: Запуск XGBoost.
        do_lstm: Запуск LSTM.
        do_tft: Запуск TFT.
        do_mv_transformer: Запуск MV-Transformer.
        show_plot: Сохранять график метрик.
        retrain: Переобучать модели.
    """
    if "do_sarimax" in kwargs:
        do_sarima = bool(kwargs.pop("do_sarimax"))
