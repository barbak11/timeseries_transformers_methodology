...
DESEASE_SPEC_KEY = SC["DESEASE_SPEC_KEY"]
...
RUN_DESEASE_TASK = SC["RUN_DESEASE_TASK"]
...
def round_pred_counts(y_pred):
    """Округление до целых счётчиков, без отрицательных."""
    return np.maximum(0, np.round(np.asarray(y_pred, dtype=float))).astype(np.int64)


def accuracy_zero_nonzero(y_true, y_pred):
    """Точность бинарной классификации: факт и прогноз «ноль / не ноль» (прогноз после round_pred_counts)."""
...
def precision_zero_nonzero(y_true, y_pred):
    """Precision положительного класса (класс 'не ноль')."""
...
def run_sparse_experiment(
    ...
    """
    Классика + PatchTST; в конце сохраняются CSV/JSON и график, таблица печатается в консоль.
    """
    setup_dirs()
    results = []
    all_predictions = []
...
def run_ets_pcr(train_df, test_df, results, all_predictions, models_dir, preds_dir):
    ...
    if len(pred_df):
        ...
        print(f"[desease][ETS_PCR] готово: {len(pred_df)} строк прогнозов")
    else:
        print("[desease][ETS_PCR] нет прогнозов")


def run_sarimax_pcr(train_df, test_df, results, all_predictions, models_dir, preds_dir):
    print("[desease][SARIMAX_PCR] старт")
    sdir = models_dir / "sarimax"
    sdir.mkdir(parents=True, exist_ok=True)
    preds_all = []
    t0 = time.perf_counter()
    ...
    if len(pred_df):
        ...
        print(f"[desease][SARIMAX_PCR] готово: {len(pred_df)} строк прогнозов")
    else:
        print("[desease][SARIMAX_PCR] нет прогнозов")


def run_xgboost_pcr(results, all_predictions, cutoff, models_dir, preds_dir):
    print("[desease][XGBoost_PCR] старт")
    ...
    if len(train_b) == 0 or len(test_b) == 0:
        ...
        return
    print(f"[desease][XGBoost_PCR] train={train_b.shape}, test={test_b.shape}")
    ...
    print(f"[desease][XGBoost_PCR] готово: {len(pred_df)} строк прогнозов")


def run_lstm_pcr(train_df, test_df, results, all_predictions, models_dir, preds_dir):
    print("[desease][LSTM_PCR] старт")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    np.random.seed(RANDOM_SEED)
    torch.manual_seed(RANDOM_SEED)
    Xw, yw = [], []
    ...
    if not Xw:
        ...
        return
    print(f"[desease][LSTM_PCR] train окон: {len(Xw)}")
    ...
    print(f"[desease][LSTM_PCR] готово: {len(pred_df)} строк прогнозов")


def run_desease_experiment(
    do_ets=True,
    do_sarimax=True,
    do_xgb=True,
    do_lstm=True,
    ...
    _artifacts = cfg.artifact_dirs(DESEASE_SPEC_KEY)
    artifacts_dir = _artifacts["base"]
    models_dir = _artifacts["models"]
    preds_dir = _artifacts["predictions"]
    metrics_dir = _artifacts["metrics"]
    ...
    if do_patchtst:
        ...
        run_patchtst_univariate(
            ...
            lr=PATCHTST_LR,
            random_seed=RANDOM_SEED,
            log_prefix="[desease][PatchTST_PCR]",
            configs=[
                {
                    "name": c["name"],
                    "seq_len": c["lookback"],
    ...
    dfm = save_metrics_files(results, DESEASE_SPEC_KEY, metrics_dir=metrics_dir)
    fig_path = metrics_dir / f"{DESEASE_SPEC_KEY}_metrics_plot.png"
    ...
    if "precision" in disp_show.columns:
        ...
        disp_show = disp_show.drop(columns=["precision"])
    print(disp_show.to_string(index=False))
    print("\nФайлы:", metrics_dir / f"{DESEASE_SPEC_KEY}_metrics.csv")
    return dfm, all_predictions
